...
?>
	...
	<?php if ( ! have_comments() ) : ?>
		...
		<p class="post-excerpt"><?php _e( 'There are no comments on this post yet. Be the first one!', 'krown' ); ?></p>
	...
	<?php endif; ?>
		...
		<?php 
			...
			$defaults = array( 'fields' => apply_filters( 'comment_form_default_fields', array(
				'author' => '<div class="krown-column-container span4" style="margin-left:0"><label for="autor">' . __('Name', 'krown') . '</span></label><input id="author" name="author" type="text" value=""/></div>',
				'email'  => '<div class="krown-column-container span4"><label for="email">' . __('Email', 'krown') . '</span></label><input id="email" name="email" type="text" value="" /></div>',
				'url'    => '<div class="krown-column-container span4 last"><label for="url">' . __('Website', 'krown') . '</span></label><input id="url" name="url" type="text" value="" /></div>' ) ),
				'comment_field' => '<div class="krown-column-container span12 last" style="margin-left:0"><label for="comment">' . __('Your Comment', 'krown') . '</span></label><textarea id="comment" name="comment" rows="8"></textarea></div>',
...
<?php
	...
	function krown_comment( $comment, $args, $depth ) {
		...
		<li id="comment-<?php comment_ID(); ?>" class="comment clearfix">
			...
			<div class="comment-avatar custom-shape hexagon-small"><?php echo get_avatar( $comment, ( isset( $retina ) && $retina === 'true' ? 130 : 65 ), $default='' ); ?></div>
